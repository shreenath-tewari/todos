require 'rails_helper'

RSpec.describe "Items API", type: :request do
  # get test data
  let!(:user) { create(:user) }
  let!(:todo) { create(:todo, created_by: user.id) }
  let!(:items) { create_list(:item, 20, todo_id: todo.id) }
  let(:todo_id) { todo.id }
  let(:id) {items.first.id}
  let(:headers) { valid_headers }

  # for index action
  describe "GET /todos/:todo_id/items" do
    # get http request
    before { get "/todos/#{todo_id}/items", params: {}, headers: headers }

    # check for tests
    context "when todo exists" do
      it "returns status code 200" do
        expect(response).to have_http_status(200)
      end

      it "returns all todo items" do
        expect(json.size).to eq(20)
      end
    end

    context "when todo does not exist" do
      let(:todo_id) { 0 }

      it "returns status code 404" do
        expect(response).to have_http_status(404)
      end

      it "returns a not found message" do
        expect(response.body).to match(/Couldn't find Todo/)
      end
    end
  end

  # for show action
  describe "GET /todos/:todo_id/items/:id" do
    # get http request
    before { get "/todos/#{todo_id}/items/#{id}", params: {}, headers: headers }

    # check for tests
    context "when todo item exists" do
      it "returns status code 200" do
        expect(response).to have_http_status(200)
      end

      it "returns todo item" do
        expect(json["id"]).to eq(id)
      end
    end

    context "when todo item does not exist" do
      let(:id) { 0 }

      it "returns status code 404" do
        expect(response).to have_http_status(404)
      end

      it "returns a not found message" do
        expect(response.body).to match(/Couldn't find Item/)
      end
    end
  end

  # for create action
  describe "POST /todos/:todo_id/items" do
    # post http request
    let(:valid_attributes) { { name: "Read", done: false, todo_id: todo_id }.to_json }

    # check for tests
    context "when request is valid" do
      # post http request
      before { post "/todos/#{todo_id}/items", params: valid_attributes, headers: headers }

      it "returns status code 201" do
        expect(response).to have_http_status(201)
      end
    end

    context "when request is invalid" do
      # invalid attributes
      before { post "/todos/#{todo_id}/items", params: {}, headers: headers }

      it "returns status code 422" do
        expect(response).to have_http_status(422)
      end

      it "returns a failure message" do
        expect(response.body).to match(/Validation failed: Name can't be blank/)
      end
    end
  end

  # for update action
  describe "PUT /todos/:todo_id/items/:id" do
    # valid attributes
    let(:valid_attributes) { { name: "Barca" }.to_json }

    # check for tests
    context "when request is valid" do
      # put http request
      before { put "/todos/#{todo_id}/items/#{id}", params: valid_attributes, headers: headers }

      it "returns status code 204" do
        expect(response).to have_http_status(204)
      end

      it "updates the item" do
        updated_item = Item.find(id)
        expect(updated_item.name).to match(/Barca/)
      end
    end

    context "when request is invalid" do
      # invalid attributes
      before { put "/todos/#{todo_id}/items/0", params: {}, headers: headers }

      it 'returns status code 404' do
        expect(response).to have_http_status(404)
      end

      it 'returns a not found message' do
        expect(response.body).to match(/Couldn't find Item/)
      end
    end
  end

  # for delete action
  describe "DELETE /todos/:todo_id/items/:id" do
    # delete http request
    before { delete "/todos/#{todo_id}/items/#{id}", params: {}, headers: headers }

    # check for tests
    it "returns status code 204" do
      expect(response).to have_http_status(204)
    end
  end
end